...
@RestController // Cette annotation indique que la classe est un contrôleur REST.REST est une manière standard de structurer les communications entre un client (comme un
// navigateur web ou une application mobile) et un serveur. Un service web RESTful permet au client de récupérer ou modifier des informations sur le serveur à travers des
// requêtes HTTP standards (comme GET pour récupérer des informations ou POST pour envoyer des informations).
